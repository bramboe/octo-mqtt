...
exports.RC2Device = exports.RC2_CHARACTERISTIC_UUID = exports.RC2_SERVICE_UUID = exports.RC2_COMMANDS = void 0;
const events_1 = require("events");
const logger_1 = require("../Utils/logger");
// RC2 Command Constants (from ESPHome analysis)
exports.RC2_COMMANDS = {
    // Movement commands
...
exports.RC2_SERVICE_UUID = 'ffe0';
exports.RC2_CHARACTERISTIC_UUID = 'ffe1';
class RC2Device extends events_1.EventEmitter {
    constructor(config, esphomeConnection) {
        super();
        ...
        (0, logger_1.logInfo)(`[RC2Device] Created device: ${config.friendlyName} (${config.address})`);
    }
    /**
    ...
    async connect() {
        try {
            (0, logger_1.logInfo)(`[RC2Device] Connecting to ${this.config.friendlyName}...`);
            // Connect via ESPHome proxy
            await this.esphomeConnection.connectToDevice({
                address: this.config.address,
                serviceUuid: exports.RC2_SERVICE_UUID,
                characteristicUuid: exports.RC2_CHARACTERISTIC_UUID
            });
            this.connected = true;
            ...
            this.startKeepAlive();
            (0, logger_1.logInfo)(`[RC2Device] Successfully connected to ${this.config.friendlyName}`);
            this.emit('connected', this.status);
        }
        catch (error) {
            (0, logger_1.logError)(`[RC2Device] Failed to connect to ${this.config.friendlyName}:`, error);
            this.connected = false;
    ...
    async disconnect() {
        try {
            (0, logger_1.logInfo)(`[RC2Device] Disconnecting from ${this.config.friendlyName}...`);
            // Stop keep-alive
            this.stopKeepAlive();
            ...
            this.status.connected = false;
            this.status.lastUpdate = new Date();
            (0, logger_1.logInfo)(`[RC2Device] Successfully disconnected from ${this.config.friendlyName}`);
            this.emit('disconnected', this.status);
        }
        catch (error) {
            (0, logger_1.logError)(`[RC2Device] Error disconnecting from ${this.config.friendlyName}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async setPosition(head, feet) {
        if (!this.connected) {
            throw new Error('Device not connected');
        }
        (0, logger_1.logInfo)(`[RC2Device] Setting position - Head: ${head}%, Feet: ${feet}%`);
        // Validate positions
        ...
        if (Math.abs(headDiff) < 0.5 && Math.abs(feetDiff) < 0.5) {
            (0, logger_1.logInfo)(`[RC2Device] Already at target position`);
            return;
        }
        ...
        (0, logger_1.logInfo)(`[RC2Device] Position set completed - Head: ${head}%, Feet: ${feet}%`);
    }
    /**
     * Set light state
     */
    async setLight(state) {
        ...
        (0, logger_1.logInfo)(`[RC2Device] Setting light: ${state ? 'ON' : 'OFF'}`);
        const command = state ? exports.RC2_COMMANDS.LIGHT_ON : exports.RC2_COMMANDS.LIGHT_OFF;
        await this.sendCommand(command);
        this.status.lightState = state;
        this.status.lastUpdate = new Date();
        this.emit('lightChanged', state);
        (0, logger_1.logInfo)(`[RC2Device] Light ${state ? 'turned on' : 'turned off'}`);
    }
    /**
    ...
    async stopAllMovement() {
        ...
        (0, logger_1.logInfo)(`[RC2Device] Stopping all movement`);
        ...
        await this.sendCommand(exports.RC2_COMMANDS.STOP);
        await new Promise(resolve => setTimeout(resolve, 100));
        await this.sendCommand(exports.RC2_COMMANDS.STOP);
        this.emit('movementStopped');
        (0, logger_1.logInfo)(`[RC2Device] All movement stopped`);
    }
    /**
     * Update calibration settings
     */
    updateCalibration(headSeconds, feetSeconds) {
        ...
        this.status.lastUpdate = new Date();
        (0, logger_1.logInfo)(`[RC2Device] Calibration updated - Head: ${this.status.calibration.head}s, Feet: ${this.status.calibration.feet}s`);
        this.emit('calibrationChanged', this.status.calibration);
    }
    /**
     * Move head in specified direction for specified duration
     */
    async moveHead(direction, durationMs) {
        const command = direction === 'up' ? exports.RC2_COMMANDS.HEAD_UP : exports.RC2_COMMANDS.HEAD_DOWN;
     ...
     */
    async moveFeet(direction, durationMs) {
        const command = direction === 'up' ? exports.RC2_COMMANDS.FEET_UP : exports.RC2_COMMANDS.FEET_DOWN;
     ...
     */
    async moveBoth(direction, durationMs) {
        const command = direction === 'up' ? exports.RC2_COMMANDS.BOTH_UP : exports.RC2_COMMANDS.BOTH_DOWN;
     ...
     */
    async executeTimedMovement(command, movementType, durationMs) {
        // Set up movement tracking
        this.movementDirection = movementType;
     ...
     */
    startPositionUpdates(movementType, totalDurationMs) {
        if (this.positionUpdateInterval) {
            clearInterval(this.positionUpdateInterval);
        }
    ...
    async sendCommand(command) {
        try {
            ...
            (0, logger_1.logDebug)(`[RC2Device] Sending command: [${command.map(b => '0x' + b.toString(16)).join(', ')}]`);
            ...
            (0, logger_1.logError)(`[RC2Device] Error sending command:`, error);
            throw error;
        }
    }
    /**
    ...
    startKeepAlive() {
        ...
        (0, logger_1.logInfo)(`[RC2Device] Starting keep-alive mechanism`);
        // Send keep-alive every 30 seconds (ESPHome pattern)
        this.keepAliveInterval = setInterval(async () => {
            try {
                if (!this.connected) {
                    (0, logger_1.logWarn)(`[RC2Device] Device not connected, skipping keep-alive`);
                    return;
                }
                ...
                (0, logger_1.logDebug)(`[RC2Device] Keep-alive sent for ${this.config.friendlyName}`);
            }
            catch (error) {
                (0, logger_1.logError)(`[RC2Device] Error sending keep-alive:`, error);
    ...
    stopKeepAlive() {
        if (this.keepAliveInterval) {
            clearInterval(this.keepAliveInterval);
            this.keepAliveInterval = null;
            (0, logger_1.logInfo)(`[RC2Device] Keep-alive stopped`);
        }
    }
    /**
    ...
    dispose() {
        ...
        (0, logger_1.logInfo)(`[RC2Device] Device ${this.config.friendlyName} disposed`);
    }
}
exports.RC2Device = RC2Device;
//# sourceMappingURL=RC2Device.js.map