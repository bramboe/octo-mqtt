...
export const connectToESPHome = async (): Promise<IESPConnection> => {
  ...
  if (validConnections.length === 0) {
    ...
    logWarn('[ESPHome] Common issues:');
    logWarn('  - ESPHome device is not powered on or not connected to network');
    logWarn('  - IP address is incorrect');
    logWarn('  - ESPHome device does not have BLE proxy configured');
    logWarn('  - Port number is incorrect (should be 6053 for ESPHome API)');
    logWarn('  - Network firewall is blocking the connection');
  } else {
