...
exports.cleanupOctoMqttEntities = exports.setupOctoMqttEntities = void 0;
...
const buildComplexCommand = ({ command, data }) => {
    ...
    const bytes = [0x40, ...command, dataLen >> 8, dataLen, 0x0, ...(data || []), 0x40].map(byte_1.byte);
...
const publishDeviceConfig = (mqtt, device, // Using placeholder
entityId, component, config) => {
...
const sendBleCommand = async (bleController, command) => {
    try {
        await bleController.writeCommand(command);
    }
    catch (error) {
        (0, logger_1.logError)('[MQTTEntities] Error sending BLE command:', error);
        throw error;
    }
};
const updateAndPublishPosition = (mqtt, storage, actuator, currentPos, deviceIdentifier) => {
...
const stopMovement = (actuatorState, mqtt, storage, actuator, bleController, // Allow null for cleanup scenario
deviceIdentifier, isCalibrationStop = false) => {
    if (actuatorState.moveTimeoutId)
        clearTimeout(actuatorState.moveTimeoutId);
    if (actuatorState.positionUpdateIntervalId)
        clearInterval(actuatorState.positionUpdateIntervalId);
    actuatorState.isMoving = false;
    actuatorState.moveTimeoutId = null;
    actuatorState.positionUpdateIntervalId = null;
    if (bleController) {
        sendBleCommand(bleController, Commands.STOP_MOVEMENT).catch((err) => (0, logger_1.logError)('Failed to send STOP command during stopMovement', err));
    }
    if (!isCalibrationStop) { // Only update position based on timing if it's not a calibration stop (where position becomes 100%)
        ...
        if (actuatorState.startTime > 0 && duration > 0) { // Ensure startTime and duration are valid
            ...
            if (actuatorState.targetPosition < actuatorState.startPosition)
                change = -change;
            ...
            (0, logger_1.logInfo)(`[MQTTEntities] ${actuator} movement stopped. Final position: ${finalPosition}%`);
        }
        else {
            ...
            (0, logger_1.logWarn)(`[MQTTEntities] ${actuator} movement stopped, but timing info was unreliable. Set to target: ${actuatorState.targetPosition}%`);
        }
    }
};
const startTimedMovement = (actuatorState, mqtt, storage, actuator, targetPos, bleController, deviceIdentifier) => {
    if (actuatorState.isMoving) {
        (0, logger_1.logWarn)(`[MQTTEntities] ${actuator} is already moving. Ignoring command.`);
        return;
    }
    ...
    if (durationFull <= 0) {
        (0, logger_1.logError)(`[MQTTEntities] ${actuator} calibration duration is not valid (${durationFull}ms). Please calibrate first.`);
        return;
    }
    ...
    if (moveDur <= 0) { // Should not happen if durationFull is >0 and there's a position difference
        (0, logger_1.logWarn)(`[MQTTEntities] Calculated move duration for ${actuator} is zero or negative. Aborting movement.`);
        actuatorState.isMoving = false;
    ...
    (0, logger_1.logInfo)(`[MQTTEntities] Starting ${actuator} movement from ${currentPos}% to ${targetPos}%. Estimated duration: ${moveDur.toFixed(0)}ms`);
    ...
    actuatorState.moveTimeoutId = setTimeout(() => {
        if (actuatorState.isMoving) {
            (0, logger_1.logWarn)(`[MQTTEntities] ${actuator} movement timed out.`);
            stopMovement(actuatorState, mqtt, storage, actuator, bleController, deviceIdentifier);
        }
    }, moveDur + 1000);
};
const setupOctoMqttEntities = (mqtt, bleController, storage, devicePin, mqttDeviceData // Explicitly pass the device data
) => {
    ...
    ['head', 'feet', 'both'].forEach(part => {
        ...
        if (!isBoth) {
            ...
            updateAndPublishPosition(mqtt, storage, part, initialPos, deviceIdentifier);
        }
        else {
        ...
        mqtt.on(`octo/${deviceIdentifier}/${part}_cover/set`, (payload) => {
            const cmd = (typeof payload === 'string' ? payload : payload.toString()).toUpperCase();
            (0, logger_1.logInfo)(`[MQTTEntities] Received command for ${part} cover: ${cmd}`);
            if (isBoth) {
                if (cmd === 'OPEN') {
                ...
                const actuator = part;
                if (cmd === 'OPEN')
                    startTimedMovement(currentActuatorState, mqtt, storage, actuator, 100, bleController, deviceIdentifier);
                else if (cmd === 'CLOSE')
                    startTimedMovement(currentActuatorState, mqtt, storage, actuator, 0, bleController, deviceIdentifier);
                else if (cmd === 'STOP')
                    stopMovement(currentActuatorState, mqtt, storage, actuator, bleController, deviceIdentifier);
            }
        });
        mqtt.subscribe(`octo/${deviceIdentifier}/${part}_cover/set_position`);
        mqtt.on(`octo/${deviceIdentifier}/${part}_cover/set_position`, (payload) => {
            const positionCmd = (typeof payload === 'string' ? payload : payload.toString());
            const position = parseInt(positionCmd, 10);
            (0, logger_1.logInfo)(`[MQTTEntities] Received set_position for ${part} cover: ${position}`);
            if (isNaN(position) || position < 0 || position > 100) {
                (0, logger_1.logWarn)(`[MQTTEntities] Invalid position value: ${positionCmd}`);
                return;
            }
            if (isBoth) {
                ...
                startTimedMovement(currentActuatorState, mqtt, storage, part, position, bleController, deviceIdentifier);
            }
        });
    });
    // --- Light ---
    ...
    mqtt.on(lightConfig.command_topic, async (payload) => {
        const cmd = (typeof payload === 'string' ? payload : payload.toString()).toUpperCase();
        (0, logger_1.logInfo)(`[MQTTEntities] Received command for light: ${cmd}`);
        ...
        try {
            ...
            (0, logger_1.logError)("Failed to send light command", e);
        }
    });
    ...
    ['head', 'feet'].forEach(part => {
        ...
        mqtt.on(calStartButtonConfig.command_topic, (payload) => {
            if ((typeof payload === 'string' ? payload : payload.toString()).toUpperCase() === 'PRESS') {
                if (headState.calibrationMode || feetState.calibrationMode) {
                    (0, logger_1.logWarn)("[MQTTEntities] Calibration already in progress.");
                    return;
                }
                (0, logger_1.logInfo)(`[MQTTEntities] Starting ${part} calibration`);
    ...
    mqtt.on(calStopButtonConfig.command_topic, (payload) => {
        if ((typeof payload === 'string' ? payload : payload.toString()).toUpperCase() === 'PRESS') {
            ...
            if (!activeCalibrationState || !activeActuator) {
                (0, logger_1.logWarn)("[MQTTEntities] No calibration in progress to stop.");
                return;
            }
            (0, logger_1.logInfo)(`[MQTTEntities] Stopping ${activeActuator} calibration`);
            ...
            if (calibrationDuration < 1000) { // Safety check for too short calibration
                (0, logger_1.logWarn)("[MQTTEntities] Calibration duration too short, not saving.");
            ...
            if (activeActuator === 'head') {
                ...
                (0, logger_1.logInfo)(`[MQTTEntities] Head calibrated. Duration: ${calibrationDuration}ms. Moving to 0%.`);
                startTimedMovement(headState, mqtt, storage, 'head', 0, bleController, deviceIdentifier);
            }
                ...
                (0, logger_1.logInfo)(`[MQTTEntities] Feet calibrated. Duration: ${calibrationDuration}ms. Moving to 0%.`);
                startTimedMovement(feetState, mqtt, storage, 'feet', 0, bleController, deviceIdentifier);
            }
        }
    });
    ...
    if (devicePin && devicePin.length === 4) {
        ...
        keepAliveIntervalId = setInterval(() => {
            (0, logger_1.logInfo)('[MQTTEntities] Sending Keep Alive');
            sendBleCommand(bleController, keepAliveFullCommand).catch(e => (0, logger_1.logError)("Failed to send keep alive", e));
        }, 30000);
    }
    else {
        (0, logger_1.logWarn)('[MQTTEntities] PIN not configured or invalid, keep-alive will not be sent.');
    }
    mqtt.publish(`octo/${deviceIdentifier}/status`, 'online');
    (0, logger_1.logInfo)(`[MQTTEntities] MQTT entities configured for ${device.name}`);
};
exports.setupOctoMqttEntities = setupOctoMqttEntities;
const cleanupOctoMqttEntities = (mqtt, deviceData) => {
    if (keepAliveIntervalId) {
        clearInterval(keepAliveIntervalId);
        keepAliveIntervalId = null;
    }
    const tempStorage = new storage_1.OctoStorage();
    if (headState.isMoving && deviceData)
        stopMovement(headState, mqtt, tempStorage, 'head', null, deviceData.identifiers[0]);
    if (feetState.isMoving && deviceData)
        stopMovement(feetState, mqtt, tempStorage, 'feet', null, deviceData.identifiers[0]);
    headState = initialActuatorState();
    feetState = initialActuatorState();
    if (deviceData) {
        mqtt.publish(`octo/${deviceData.identifiers[0]}/status`, 'offline');
    }
    (0, logger_1.logInfo)('[MQTTEntities] Cleaned up MQTT entities and keep-alive.');
};
exports.cleanupOctoMqttEntities = cleanupOctoMqttEntities;
//# sourceMappingURL=mqttEntities.js.map