    ...
    async startScan() {
        try {
            ...
            const response = await fetch(apiUrl('/scan/start'), {
                ...
                body: JSON.stringify({
                    clientInfo: 'Octo MQTT Web UI v2.6.4',
                    timestamp: timestamp,
...
(function() {
  ...
  async function selfTest() {
    ...
    try {
      ...
      logDiag('Health: ' + health.status);
    } catch (e) {
      logDiag('Health check failed: ' + e, 'red');
    }
    try {
      const scan = await fetch(apiUrl('/scan/status'));
      logDiag('Scan status: ' + scan.status);
    } catch (e) {
      logDiag('Scan status failed: ' + e, 'red');
    }
  }

  // Main UI logic (BLEScannerApp) with error logging
  class BLEScannerApp {
    constructor() {
      this.startScanBtn = document.getElementById('start-scan');
      this.stopScanBtn = document.getElementById('stop-scan');
  ...
  // Initialize app when DOM is loaded, with error logging
